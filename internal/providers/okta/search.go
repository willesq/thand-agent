...
// buildSearchIndex builds the Bleve search index for Okta permissions and roles
func (p *oktaProvider) buildSearchIndex() {
	startTime := time.Now()
	defer func() {
		elapsed := time.Since(startTime)
		logrus.Debugf("Built Okta search indices in %s", elapsed)
	}()
	...
	logrus.WithFields(logrus.Fields{
		...
		"roles":       len(p.roles),
	}).Debug("Okta search indices ready")
}
